...
def search_for_movies(query, topn=10, w_review = 1, w_syn = 1, w_r_d=1, w_r_f=1, w_r_l=1, w_r_t=1):
   ...
   # review_pred = doc2vec.predict_movies(input_list = query_list, model_path='data/doc2vec_reviews.model')
    print(np.sort(doc2vec.predict_movies(input_list = query_list, model_path='data/doc2vec_reviews.model')))
